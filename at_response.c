...
EXPORT_DEF const at_responses_t at_responses = {
    at_responses_list, 2, ITEMS_OF(at_responses_list), RES_MIN, RES_MAX};
...
static int at_response_rcend(struct pvt *pvt, const char *str) {
  ...
  if (cpvt) {
    ...
    ast_debug(1,
              "[%s] CEND: call_index %d duration %d end_status %d cc_cause %d "
              "Line disconnected\n",
  ...
  if (ecmd->res == RES_OK || ecmd->res == RES_CMGR) {
    ...
    case CMD_AT_CVOICE2:
      ...
      static const at_queue_cmd_t cmds1[] = {
          ATQ_CMD_DECLARE_STIT(CMD_AT_Z, cmd_atrcend, ATQ_CMD_TIMEOUT_MEDIUM,
                               0),
      };
      if (at_queue_insert_const(&pvt->sys_chan, cmds1, ITEMS_OF(cmds1), 1) !=
          0) {
      ...
      /*
                              case CMD_AT_CLIP:
                                      ast_debug (1, "[%s] Calling line
         indication disabled\n", PVT_ID(pvt)); break;
      */
    case CMD_AT_CSSN:
      ast_debug(1,
                "[%s] Supplementary Service Notification enabled successful\n",
                PVT_ID(pvt));
      break;
    ...
    case CMD_AT_CHLD_2x:
      ...
      */
      ...
      ast_debug(1, "[%s] %s sent successfully for call id %d\n", PVT_ID(pvt),
                at_cmd2str(ecmd->cmd), task->cpvt->call_idx);
      break;
    ...
    case CMD_AT_CMGR:
      ...
      at_retrieve_next_sms(&pvt->sys_chan,
                           at_cmd_suppress_error_mode(ecmd->flags));
      break;
...
static void log_cmd_response_error(const struct pvt *pvt,
                                   const at_queue_cmd_t *ecmd, const char *fmt,
                                   ...) {
  va_list ap;
  char tempbuff[512];
...
static int at_response_error(struct pvt *pvt, at_res_t res) {
  ...
  if (ecmd && (ecmd->res == RES_OK || ecmd->res == RES_CMGR ||
    ...
    case CMD_AT_CGMI:
      log_cmd_response_error(
          pvt, ecmd, "[%s] Getting manufacturer info failed\n", PVT_ID(pvt));
      goto e_return;
    ...
    case CMD_AT_CMEE:
      log_cmd_response_error(pvt, ecmd,
                             "[%s] Setting error verbosity level failed\n",
                             PVT_ID(pvt));
      goto e_return;
    ...
    case CMD_AT_COPS_INIT:
      log_cmd_response_error(pvt, ecmd,
                             "[%s] Error setting operator select parameters\n",
                             PVT_ID(pvt));
      goto e_return;

    case CMD_AT_CREG_INIT:
      log_cmd_response_error(
          pvt, ecmd, "[%s] Error enabling registration info\n", PVT_ID(pvt));
      goto e_return;
    ...
    case CMD_AT_CVOICE2:
      ...
      if (!pvt->initialized) {
        ...
        if (at_enqueue_initialization(task->cpvt, CMD_AT_CMGF)) {
          log_cmd_response_error(
              pvt, ecmd, "[%s] Error schedule initialization commands\n",
              PVT_ID(pvt));
          goto e_return;
        }
      }
      ...
      /*
                              case CMD_AT_CLIP:
                                      log_cmd_response_error(pvt, ecmd, "[%s]
         Error enabling calling line indication\n", PVT_ID(pvt)); goto e_return;
      */
    case CMD_AT_CSSN:
      log_cmd_response_error(
          pvt, ecmd,
          "[%s] Error Supplementary Service Notification activation failed\n",
          PVT_ID(pvt));
      goto e_return;
      ...
      if (!pvt->initialized) {
        if (pvt->has_voice) {
          ...
          if (at_enqueue_initialization(task->cpvt, CMD_AT_CSQ)) {
            log_cmd_response_error(pvt, ecmd,
                                   "[%s] Error querying signal strength\n",
                                   PVT_ID(pvt));
            goto e_return;
          }
    ...
    case CMD_AT_CHLD_3:
      log_cmd_response_error(pvt, ecmd,
                             "[%s] Can't begin conference call idx %d\n",
                             PVT_ID(pvt), task->cpvt->call_idx);
      queue_hangup(task->cpvt->channel, 0);
      break;
    ...
    case CMD_AT_DDSETEX:
      log_cmd_response_error(pvt, ecmd,
                             "[%s] %s Enable audio failed, retrying...\n",
                             PVT_ID(pvt), at_cmd2str(ecmd->cmd));
    ...
    case CMD_AT_CHLD_1x:
      log_cmd_response_error(pvt, ecmd,
                             "[%s] Error sending hangup for call idx %d\n",
                             PVT_ID(pvt), task->cpvt->call_idx);
      break;

    case CMD_AT_CMGR:
      //				log_cmd_response_error(pvt, ecmd, "[%s]
      // Error reading SMS message, resetting index\n", PVT_ID(pvt));
    ...
    case CMD_AT_SMSTEXT:
      ...
      log_cmd_response_error(pvt, ecmd,
                             "[%s] Error sending SMS message %p %s\n",
    ...
    default:
      log_cmd_response_error(
          pvt, ecmd, "[%s] Received 'ERROR' for unhandled command '%s'\n",
          PVT_ID(pvt), at_cmd2str(ecmd->cmd));
      break;
    }
    at_queue_handle_result(pvt, res);
    ...
    default:
      log_cmd_response_error(
          pvt, ecmd, "[%s] Received 'ERROR' when expecting '%s', ignoring\n",
          PVT_ID(pvt), at_res2str(ecmd->res));
      break;
    }
  } else {
...
static void request_clcc(struct pvt* pvt)
{
        if (at_enqueue_clcc(&pvt->sys_chan))
        {
                ast_log(LOG_ERROR, "[%s] Error enqueue List Current Calls
request\n", PVT_ID(pvt));
        }
}
*/
...
static int at_response_orig(struct pvt *pvt, const char *str) {
  ...
  if (sscanf(str, "^DSCI:%d,%*d,3,%d,%*s", &call_index, &call_type) == 2 &&
    ...
    if (call_type == CLCC_CALL_TYPE_VOICE) {
      ...
      if (cpvt) {
        ...
        ast_log(LOG_ERROR,
                "[%s] answered incoming call with not exists call idx %d, "
                "hanging up!\n",
                PVT_ID(pvt), call_index);
      }
    } else
      ast_log(
          LOG_ERROR,
          "[%s] answered not voice incoming call type '%d' idx %d, skipped\n",
          PVT_ID(pvt), call_type, call_index);
    return 0;
  }

  if (sscanf(str, "^DSCI:%d,%*d,6,%d,%*s", &call_index, &call_type) == 2 &&
      call_type == 0) {
    int duration = 0;
    int end_status = 0;
    int cc_cause = 0;
    ...
    if (cpvt) {
      if (pvt->call_estb)
        duration = uptime() - pvt->t0;
      pvt->call_estb = 0;
      CPVT_RESET_FLAGS(cpvt, CALL_FLAG_NEED_HANGUP);
      PVT_STAT(pvt, calls_duration[cpvt->dir]) += duration;
      change_channel_state(cpvt, CALL_STATE_RELEASED, cc_cause);
      manager_event_cend(PVT_ID(pvt), call_index, duration, end_status,
                         cc_cause);
    } else {
      //		ast_log (LOG_ERROR, "[%s] CEND event for unknown call
      // idx '%d'\n", PVT_ID(pvt), call_index);
    }
  ...
  if (call_type == CLCC_CALL_TYPE_VOICE) {
      ...
      /*			if(pvt->volume_sync_step == VOLUME_SYNC_BEGIN)
                              {
                                      ...
                                      {
                                              ast_log (LOG_ERROR, "[%s] Error
         synchronize audio level\n", PVT_ID(pvt));
                                      }
                                      else
                                              pvt->volume_sync_step++;
                              }
     ...
     */
    ast_log(LOG_ERROR,
            "[%s] ORIG event for non-voice call type '%d' index %d\n",
            PVT_ID(pvt), call_type, call_index);
  }
  return 0;
}
...
static int start_pbx(struct pvt *pvt, const char *number, int call_idx,
                     call_state_t state) {
  struct cpvt *cpvt;
  ...
  struct ast_channel *channel = new_channel(
      ...
      pvt->has_subscriber_number ? pvt->subscriber_number
...
static int at_response_clcc(struct pvt *pvt, char *str) {
  ...
  if (pvt->initialized) {
    ...
    for (;;) {
      p = strchr(str, '\r');
      if (at_parse_clcc(str, &call_idx, &dir, &state, &mode, &mpty, &number,
                        &type) == 0) {
        ast_debug(3,
                  "[%s] CLCC callidx %u dir %u state %u mode %u mpty %u number "
                  "%s type %u\n",
        ...
        if (mode == CLCC_CALL_TYPE_VOICE && state <= CALL_STATE_WAITING) {
          ...
          if (cpvt) {
            ...
            if (dir == cpvt->dir) {
              ...
              if (dir == CALL_DIR_INCOMING && (state == CALL_STATE_INCOMING ||
              ...
              ast_log(LOG_ERROR,
                      "[%s] CLCC call idx %d direction mismatch %d/%d\n",
                      PVT_ID(pvt), cpvt->call_idx, dir, cpvt->dir);
            }
          } else if (dir == CALL_DIR_INCOMING &&
            ...
            if (pvt_enabled(pvt)) {
              /* TODO: give dialplan level user tool for checking device is
               * voice enabled or not  */
              if (start_pbx(pvt, number, call_idx, state) == 0) {
                PVT_STAT(pvt, in_calls_handled)++;
                if (!pvt->has_voice)
                  ast_log(LOG_WARNING,
                          "[%s] pbx started for device not voice capable\n",
                          PVT_ID(pvt));
              } else
    ...
    if (all == held) {
      /* HW BUG 2: when no active call exists not way to enable voice again on
         activated from hold call call will be activated but no voice
      */
...
static int at_response_ccwa(struct pvt *pvt, char *str) {
   ...
   *validity>[,<subaddr>,<satype>[,<priority>]]]
   *
   */
  ...
  if (n == 1)
    ...
    if ((class & CCWA_CLASS_VOICE) &&
        (status == CCWA_STATUS_NOT_ACTIVE || status == CCWA_STATUS_ACTIVE)) {
  ...
  if (pvt->initialized) {
    //		if (sscanf (str, "+CCWA: \"%*[+0-9*#ABCabc]\",%*d,%d", &class)
    //== 1)
    if (at_parse_ccwa(str, &class) == 0) {
      //			if (CONF_SHARED(pvt, callwaiting) !=
      // CALL_WAITING_DISALLOWED && class == CCWA_CLASS_VOICE)
      if (class == CCWA_CLASS_VOICE) {
        pvt->rings++;
        pvt->cwaiting = 1;
        request_clcc(pvt);
...
static int at_response_ring(struct pvt *pvt) {
  ...
  if (pvt->initialized) {
    ...
    /* We only want to syncronize volume on the first ring and if no channels
     * yes */
    /*		if (pvt->volume_sync_step == VOLUME_SYNC_BEGIN &&
       PVT_NO_CHANS(pvt))
                    {
                            ...
                            {
                                    ast_log (LOG_ERROR, "[%s] Error synchronize
       audio level\n", PVT_ID(pvt));
                            }
                            else
                                    pvt->volume_sync_step++;
  ...
  if (CONF_SHARED(pvt, disablesms) == 0) {
    ...
    for (i = 0; i != SMS_INDEX_MAX; i++) {
      if (at_enqueue_retrieve_sms(&pvt->sys_chan, i, SUPPRESS_ERROR_ENABLED)) {
        ast_log(LOG_ERROR,
                "[%s] Error sending CMGR to retrieve SMS message #%d\n",
                PVT_ID(pvt), i);
        return -1;
      }
    }
...
static int at_response_cmti(struct pvt *pvt, const char *str) {
  ...
  if (CONF_SHARED(pvt, disablesms)) {
    ast_log(LOG_WARNING,
            "[%s] SMS reception has been disabled in the configuration.\n",
            PVT_ID(pvt));
    return 0;
  }
  ...
  if (index > -1) {
    ...
    /* Not sure why this happens, but we don't want to disconnect standing
     * calls. [Jun 14 19:57:57] ERROR[3056]: at_response.c:1173
     * at_response_cmti: [m1-1] Error parsing incoming sms message alert '+CMTI:
     * "SM",-1' */
    ast_log(LOG_WARNING,
            "[%s] Error parsing incoming sms message alert '%s', ignoring\n",
            PVT_ID(pvt), str);
  }
...
static int at_response_cmgr(struct pvt *pvt, const char *str, size_t len) {
  ...
  if (ecmd) {
    if (ecmd->res == RES_CMGR || ecmd->cmd == CMD_USER) {
      ...
      res = at_parse_cmgr(str, len, &tpdu_type, sca, sizeof(sca), oa,
                          sizeof(oa), scts, &mr, &st, dt, msg, &msg_len, &udh);
      if (res < 0) {
      ...
      case PDUTYPE_MTI_SMS_STATUS_REPORT:
        ast_verb(
            1,
            "[%s] Got status report with ref %d from %s and status code %d\n",
            PVT_ID(pvt), mr, oa, st);
        snprintf(buf, 64, "Delivered\r\nForeignID: %d", mr);
        payload_len = smsdb_outgoing_part_status(pvt->imsi, oa, mr, st,
        ...
        if (payload_len >= 0) {
          ...
          ast_verb(1, "[%s] Success: %d; Payload: %.*s; Report string: %s\n",
                   PVT_ID(pvt), success, (int)payload_len, payload,
                   status_report_str);
          ...
          manager_event_report(PVT_ID(pvt), payload, payload_len, scts, dt,
                               success, 1, status_report_str);
        }
        break;
      case PDUTYPE_MTI_SMS_DELIVER:
         ...
         */
        ast_verb(
            1, "[%s] Got SM part from %s: '%s'; [ref=%d, parts=%d, order=%d]\n",
            PVT_ID(pvt), oa, msg, udh.ref, udh.parts, udh.order);
        if (sms_queue_put(pvt->imsi, PVT_ID(pvt), oa, scts, udh.ref, udh.parts,
                          udh.order, msg) != 0) {
          ast_log(LOG_ERROR, "[%s] Failed to queue SMS for async processing\n",
                  PVT_ID(pvt));
        }
        /* Continue to receive next SMS immediately - no waiting for DB
         * operations */
        break;
      }
    } else {
      ast_log(LOG_ERROR,
              "[%s] Received '+CMGR' when expecting '%s' response to '%s', "
              "ignoring\n",
...
static int at_response_sms_prompt(struct pvt *pvt) {
  ...
  if (ecmd && ecmd->res == RES_SMS_PROMPT) {
    ...
    ast_log(LOG_ERROR,
            "[%s] Received sms prompt when expecting '%s' response to '%s', "
            "ignoring\n",
            PVT_ID(pvt), at_res2str(ecmd->res), at_cmd2str(ecmd->cmd));
  } else {
...
static int at_response_cusd(struct pvt *pvt, char *str, size_t len) {
  ...
  ast_verb(1, "[%s] Got USSD type %d '%s': '%s'\n", PVT_ID(pvt), type, typestr,
           cusd_utf8_str);
  ast_base64encode(text_base64, (unsigned char *)cusd_utf8_str, res,
                   sizeof(text_base64));
...
/*!
 * \brief Handle ^SMMEMFULL response This event notifies us, that the sms
 * storage is full
 * \param pvt -- pvt structure
 * \retval  0 success
 * \retval -1 error
...
/*!
 * \brief Handle +CSQ response Here we get the signal strength and bit error
 * rate
 * \param pvt -- pvt structure
 * \param str -- string containing response (null terminated)
 * \param len -- string lenght
...
static int at_response_cnum(struct pvt *pvt, char *str) {
  ...
  if (number) {
    ast_copy_string(pvt->subscriber_number, number,
...
static int at_response_cops(struct pvt *pvt, char *str) {
  ...
  if (provider_name) {
    ast_copy_string(pvt->provider_name, provider_name,
                    sizeof(pvt->provider_name));
    return 0;
  }
...
static int at_response_creg(struct pvt *pvt, char *str, size_t len) {
  ...
  if (lac) {
    ast_copy_string(pvt->location_area_code, lac,
                    sizeof(pvt->location_area_code));
  }
...
static void at_response_busy(struct pvt *pvt,
...
int at_response(struct pvt *pvt, const struct iovec iov[2], int iovcnt,
                at_res_t at_res) {
  char *str;
  size_t len;
  const at_queue_task_t *task = at_queue_head_task(pvt);
  ...
  if (iov[0].iov_len + iov[1].iov_len > 0) {
    ...
    case RES_CMGS: {
      ...
      char dst[SMSDB_DST_MAX_LEN];
      ssize_t payload_len =
          smsdb_outgoing_part_put(task->uid, res, dst, payload);
      if (payload_len >= 0) {
