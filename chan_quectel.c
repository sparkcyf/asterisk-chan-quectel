...
static snd_pcm_t *alsa_card_init(char *dev, snd_pcm_stream_t stream,
  ...
  if (err < 0) {
    ...
    ast_debug(1, "Opening device %s in %s mode\n", dev,
              (stream == SND_PCM_STREAM_CAPTURE) ? "read" : "write");
  }
...
static int soundcard_init(struct pvt *pvt) {
  ...
  pvt->ocard =
      alsa_card_init(CONF_UNIQ(pvt, alsadev), SND_PCM_STREAM_PLAYBACK, pvt);
...
static void pvt_start(struct pvt *pvt) {
  ...
  if (strcmp(CONF_UNIQ(pvt, quec_uac), "1") == 0) {
    ...
    pvt->audio_fd =
        opentty(PVT_STATE(pvt, audio_tty), &pvt->alock, pvt->is_simcom);
